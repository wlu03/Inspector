...
- ✅ **Static dashboard (aggregator) + shared design system** — `inspector/dashboard/` (`aggregate.py` pure scan/stats/recurring-bugs/fix-prompt over the trace tree; `render.py` self-contained `dashboard.html`; `build.py` + `python -m inspector.dashboard`). One replayable index of every session, newest-first, with pass-rate, severity rollups, and bugs that recur across runs; each row links its existing per-session replay (works from `file://`, no server). New shared `inspector/theme.py` lifts the landing page's design tokens (Geist + Geist Mono + Playfair Display, #111 dot-grid, #15C78D/#ff4040) — **both** the dashboard and the replay import it so they never drift.
- ✅ **Live agent fix loop (cross-session)** — MCP tools `build_dashboard`, `list_runs`, `get_run`, `fix_finding` (returns a ready-to-apply fix prompt: summary/expected-vs-actual/file:line/repro/evidence); `update_finding_status` now writes to any session on disk (live or finished) so past runs can be signed off. MCP surface now 18 tools.
- ✅ **Replay video upgraded** — downscaled (`_MAX_VIDEO_W=720`, display max-width 460) so files stay small, and each action's BEFORE frame gets a **cursor arrow + click-intent label** ("click #2" / `type "…"` / "press Escape") derived from the timeline; `Session.act` now records the *resolved* click pixel into the trace so the cursor draws even for `target_id` clicks.
- ✅ Tests: `tests/test_dashboard.py` (13 new — aggregate/stats/recurring/fix-prompt/status write-back/render/build + replay overlay). Suite **184 passing**.
- ✅ **Localhost dashboard server + auto-link** — `dashboard/serve.py` (stdlib `ThreadingHTTPServer` on 127.0.0.1:`dashboard_port`, singleton daemon thread). `test_app`/`stop`/`test_report` and `open_dashboard`/`build_dashboard` rebuild + serve and return a clickable `http://127.0.0.1:7321/dashboard.html#<sid>` (deep-link highlights the run); `replay_url` jumps into the replay. Finished workflow: test → clickable link.
- ✅ **MCP QoL batch** — (1) **tool annotations** (`READ_ONLY`/`WRITE`/`DESTRUCTIVE` via `ToolAnnotations`): safe reads auto-approve, only `launch_app`/`test_app`/`stop` (billed sandboxes) prompt. (2) **Friendly errors** — `_friendly` decorator turns a raw `KeyError` from an unknown/expired session into `{error, active_sessions, hint}`. (3) **Progress streaming** — `launch_app`/`test_app` are async with `ctx`, emitting a liveness heartbeat ("still working (Ns)") + `report_progress` through `_run_with_heartbeat` (blocking work off the loop via `anyio.to_thread`). (4) **MCP logging** — silent `except: pass` → `log.warning(exc_info=True)`. (5) **Resources** — `inspector://sessions`, `…/{sid}/report` (markdown), `…/{sid}/findings`. (10) **Session aliases** — `alias` on launch_app/test_app, resolved by `SessionManager.get`, shown in dashboard rows. (11) **Desktop notification** — `notify.py` pings (with the link) when an autonomous run finishes. (12) **Dashboard auto-refresh** — polls `dashboard.json`, shows a "new runs available" banner. MCP surface now **22 tools + 3 resources**.
- ✅ Tests: `tests/test_qol.py` (25) + `tests/test_dashboard_serve.py` (6). Suite **220 passing**.
- ✅ **Fix loop closed: Bug Ledger + auto-verify** — `aggregate.bug_ledger()` dedups findings by signature per repo and derives status from evidence (gone from the latest run → `verified`/fixed; present → `open`; `dismissed` overlay). `latest_update()` = fixed/new/still-open delta vs the prior run. New dashboard **tabs** (Runs · Bug Ledger) with an open-count badge, status badges, and a "what changed" panel; `bug_ledger` MCP tool; `dashboard.json` carries `ledger`+`update`; finish messages append "N fixed since last run, M new".
- ✅ **Live dashboard feed** — `serve.py` GET `/live.json` (pluggable provider off the live SessionManager) → pulsing "Running now" cards (findings/frames tick up mid-run), ticking relative timestamps, auto-reload when a run finishes.
- ✅ **Devin auto-fix** — `inspector/devin.py` (stdlib urllib, no dep): `fix_with_devin` starts a Devin v1 session (POST /v1/sessions, `max_acu`-capped) to open a PR and tags the issue `fixing`+`devin_url`; `poll_devin` (GET /v1/session/{id}) records `pr_url`. MCP tools `fix_with_devin`/`devin_status`; serve.py POST `/api/devin-fix|devin-status` action handler; **"Fix with Devin" button on every Bug Ledger row** with a client-side 10-min poll cap; re-run auto-verifies. Devin is also an MCP *client* — it can add Inspector as a custom MCP (needs HTTP transport) to test its own fixes.
- ✅ Tests: `tests/test_ledger.py` (5) + `tests/test_devin.py` (11) + live-feed/serve tests. Suite **265 passing**.
- ✅ **HTTP/SSE transport (Devin → Inspector)** — `server.main()` now takes `--http`/`--transport` (+ `INSPECTOR_TRANSPORT`/`HTTP_HOST`/`HTTP_PORT`/`HTTP_PATH`); `python -m inspector.server --http` serves MCP at `http://host:port/mcp` (smoke-tested: initialize → 200). Lets a remote client (Devin's MCP marketplace, HTTP transport) call `test_app`/`get_findings`/`bug_ledger` to verify its own fixes. Connect steps + "how to get a Devin API key" in [docs/16](docs/16-devin-integration.md); `.env.example` updated. Suite **266 passing**.
- ⏳ Next: auth on the HTTP transport (token/OAuth before exposing via tunnel) · per-step before/after structured assertion (STEP_PASS/FAIL) · diff-driven `test_changes` (git diff → screens) · viewport/404/keyboard primitives · ui-test-style HTML report · determinism (#28) · dev-server `.env` (#29) · Android/iOS planes · dashboard v2 (hosted/object-storage, trends/flakiness, rrweb DOM replay, live E2B stream URL).
