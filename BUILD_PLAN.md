...
- ⏳ Next: per-step before/after structured assertion (STEP_PASS/FAIL) · diff-driven `test_changes` (git diff → screens) · viewport/404/keyboard primitives · ui-test-style HTML report · broaden FallbackDriver (catch empty-type/bad actions) · determinism (#28) · dev-server `.env` (#29) · Android/iOS planes · dashboard v2 (hosted/object-storage, trends/flakiness, rrweb DOM replay, live E2B stream URL).
