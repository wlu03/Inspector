...
- **P1 — `MacOSPlane` transport.** Fill the scaffold (`run_sync`/`run_bg`/`upload`/`start`/`screenshot`/`stop`); add `macos_*` config via the existing `_env()` `INSPECTOR_*` config. Smoke-test with `macos_host=localhost` to iterate without paying tart boot. **Deliverable:** working clone/boot/ip + ssh/scp + base64 screenshot.
