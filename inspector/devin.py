...
def build_session_fix_prompt(findings: list[dict], repo_path: str, surface: str = "",
                             repo: str | None = None) -> str:
    """One prompt to fix EVERY finding from a single run (the per-session button). Pure."""
    repo = repo or github_repo(repo_path)
    where = f"In the GitHub repository **{repo}**" if repo else "In the repository above"
    rel = ""
    if repo and repo_path and os.path.isdir(repo_path):
        try:
            root = subprocess.run(
    ...
    hint = _SOURCE_HINT.get((surface or "").lower(), "the application source")
    items = "\n".join(
        f"{i}. [{f.get('severity', '?')}] {f.get('summary', '')}\n"
        f"   Expected: {f.get('expected') or '(n/a)'}\n"
        f"   Actual:   {f.get('actual') or '(n/a)'}\n"
        f"   Where:    {f.get('suspected_area') or '(grep for the text above)'}"
        for i, f in enumerate(findings, 1)
    )
    return (
        f"Inspector found {len(findings)} bug(s) in {repo_path or 'the app'} ({surface}) "
        f"during one test run. Fix ALL of them:\n\n{items}\n\n"
        f"{where} — the affected app is under {area}.\n"
        f"LOCATING THE CODE: 'Where' may be a behavioural description, not a path — grep "
        f"{area} for the on-screen text/labels/identifiers in each item ({hint}).\n"
        "THEN: implement a minimal, focused fix for EACH root cause; build and run the "
        "project's tests; and OPEN A SINGLE PULL REQUEST addressing all of them, with a "
        "clear title and a description listing each bug (severity, expected vs actual), the "
        "fix, and how you verified it. Keep the diff minimal — do not refactor unrelated "
        "code or modify example/test fixtures beyond the fixes."
    )


def fix_session_with_devin(cfg, trace_root: str, session_id: str, _api_fn=None) -> dict:
    """Hand EVERY (non-dismissed) finding from one run to Devin as a single PR task."""
    _api_fn = _api_fn or _api   # _api is defined further down; resolve at call time
    if not cfg.devin_api_key:
        return {"error": "DEVIN_API_KEY not set — add it to enable Fix with Devin"}
    from .dashboard.aggregate import load_session_detail
    detail = load_session_detail(trace_root, session_id)
    sess = detail.get("session") or {}
    if not sess:
        return {"error": f"no session {session_id!r} on disk"}
    findings = [f for f in detail.get("findings", [])
                if f.get("status") != "dismissed" and f.get("summary")]
    if not findings:
        return {"error": f"session {session_id!r} has no fixable findings"}

    repo_path, surface = sess.get("repo_path"), sess.get("surface", "")
    prompt = build_session_fix_prompt(findings, repo_path, surface,
                                      repo=getattr(cfg, "devin_repo", None))
    title = f"Fix {len(findings)} bug(s) from {sess.get('alias') or session_id}"[:120]
    try:
    ...
    devin_url, devin_session_id = resp.get("url", ""), resp.get("session_id", "")
    import glob
    for p in glob.glob(os.path.join(trace_root, session_id, "findings", "*.json")):
        try:
            patch_finding(p, {"status": "fixing", "devin_url": devin_url,
                              "devin_session_id": devin_session_id})
        except Exception:
            pass
    return {"session_id": session_id, "n_findings": len(findings), "devin_url": devin_url,
            "devin_session_id": devin_session_id, "status": "fixing",
            "note": "Devin is opening one PR for all of this run's findings."}
